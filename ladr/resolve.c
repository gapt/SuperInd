...
/********************** Modif *****************/
	...
	init_rank(r);
	if ((get_rank(nuc) != -1 && get_rank(r) > get_rank(nuc) +1) || (get_rank(sat) != -1 && get_rank(r) > get_rank(sat) +1)) {
	...
	if (get_rank(r) < get_rank(nuc) || get_rank(r) < get_rank(sat))  {
	...
	// if it's not an (alpha-C)-clause, r will be deleted in cl_process
	if (!alpha_C_clause(r)) {
		r->rank=-3;
	}
	//add_ancestors(nuc, sat, r);	
...
{   //Modif
	/*if(param_term(lit->atom)){ //|| !succ_occurs_in(lit->atom))
		printf("not parent :");
		p_clause(lit->atom->container);
		printf(" F");
		return FALSE;
	}
	else{*/
		...
		if (res_type == POS_RES) {  /* positive resolution (one parent positive) */
			...
			/* no selected literals in clause */
				...
				//printf("lit = ");p_term(lit->atom);
				//printf("MAX = %d ", maximal_literal(c->literals, lit, check_type));
				//printf("Ordered = %d ", !Ordered);
				return !Ordered || maximal_literal(c->literals, lit, check_type);
			}
		}
	//}
...
/* PUBLIC */
/*void normalization(Topform c, void (*proc_proc) (Topform))
{
	Literals l1;
	int i = 1;
	Context subst = get_context();
